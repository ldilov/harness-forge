...
function renderSharedRuntimeReadme(runtime: PersistedSharedRuntimeDocument): string {
  const lines = [
    ...
    `- \`.hforge/runtime/recursive/${RUNTIME_RECURSIVE_SESSIONS_DIR}/RS-XXX/root-frame.json\` - Compact environment-first root frame used by recursive typed execution`,
    `- \`.hforge/runtime/recursive/${RUNTIME_RECURSIVE_SESSIONS_DIR}/RS-XXX/iterations/\` - Durable typed RLM iterations with frames and action bundles`,
